...
void draw() {
  ...
  for (int i = -300; i <= -50; i +=5) {
    for (int j = 300; j >=50; j -=5) {
      ...
      float offset = 5;
      float size = 250.0;
      vertex(i, j, -300, u/size, v/size);
      vertex(i + offset, j, -300, u/size + offset/size, v/size);
      vertex(i + offset, j + offset, -300, u/size + offset/size, v/size + offset/size);
      vertex(i, j + offset, -300, u/size, v/size + offset/size);
      endShape();
