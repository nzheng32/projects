...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    for i in range(0, len(data)):
        if i % batch_size != 0 or i==0:
            currr_data.append(data[index_list[i]])
            curr_label.append(label[index_list[i]])
        else:

            batched_data.append(np.array(currr_data))
            batched_label.append(curr_label)
            currr_data = []
            curr_label =[]
            currr_data.append(data[index_list[i]])
            curr_label.append(label[index_list[i]])

        if i == len(data) - 1 and len(currr_data) > 0:
            batched_data.append(np.array(currr_data))
            batched_label.append(curr_label)
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    x = np.arange(0, num_of_epoch, 1)
    fig, ax = plt.subplots()
    line1 = ax.plot(x, train_loss_history, label="train")
    line3 = ax.plot(x, valid_loss_history, label ="Valid")
    ...
    fig, ax = plt.subplots()
    line2 = ax.plot(x, train_acc_history, label ="train")
    line4 = ax.plot(x, valid_acc_history, label ="Valid")
    ax.legend(loc ="center right")
    ax.set(xlabel='Epochs', ylabel ='Loss', title = 'accuracy Curve')
    ax.grid()
    plt.show()
