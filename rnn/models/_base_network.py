...
class _baseNetwork:
    ...
    def softmax(self, scores):
        ...
        #############################################################################
        # e^z / summation(e^z)
        prob = (np.exp(scores)) / (np.array([np.sum((np.exp(scores)), axis = 1)]).T)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def cross_entropy_loss(self, x_pred, y):
        ...
        for i in range(x_pred.shape[0]):
            index = y[i]
            loss = loss - np.log(x_pred[i][index])
    ...
    def compute_accuracy(self, x_pred, y):
        ...
        #############################################################################
        acc = 1.0*(sum(np.argmax(x_pred, axis = 1) == y)) / (1.0*x_pred.shape[0])
