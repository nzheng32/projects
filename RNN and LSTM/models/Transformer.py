...
class TransformerTranslator(nn.Module):
    ...
    def __init__(self, input_size, output_size, device, hidden_dim=128, num_heads=2, dim_feedforward=2048, dim_k=96, dim_v=96, dim_q=96, max_length=43):
        ...
        ##############################################################################
        self.token_embed = nn.Embedding(num_embeddings = self.input_size + self.max_length, embedding_dim = self.hidden_dim)
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
    ...
    def embed(self, inputs):
        ...
        #############################################################################
        embeddings = self.token_embed(inputs)
        positional_encodings = torch.LongTensor([i+self.input_size for i in range(inputs.shape[1])]).to(self.device)
        positional_encodings = self.token_embed(positional_encodings.repeat(inputs.shape[0], 1))
