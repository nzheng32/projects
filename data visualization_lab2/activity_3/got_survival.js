...
function updateBars(house) {
    if (house == 'top') {
        var filterData = characters.filter( character => character.power_ranking > 0);
    } else {
        var filterData = characters.filter( character => character.house == house);
    }
    filterData.sort(function(a, b) {
  var nameA = a["probability_of_survival"];
  var nameB = b["probability_of_survival"] // ignore upper and lowercase
  if (nameA < nameB) {
    return 1;
  }
  if (nameA > nameB) {
