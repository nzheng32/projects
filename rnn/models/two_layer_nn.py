...
class TwoLayerNet(_baseNetwork):
    ...
    def forward(self, X, y, mode='train'):
        ...
        sig= model.sigmoid(Z1)
        Z2 = np.dot(sig,self.weights['W2']) + self.weights['b2']
        output = model.softmax(Z2)
        loss = model.cross_entropy_loss(output, y)
        accuracy = model.compute_accuracy(output, y)
        ...
        ds[range(N), y] -= 1
        ds /= N
        self.gradients['W2'] = np.dot(np. transpose(sig), ds)
        self.gradients['b2'] = np.sum(ds, axis = 0)
        hide = np.dot(ds, np.transpose(self.weights['W2']))
        grad_sig = sig *(1- sig)
        grad = hide * grad_sig
        self.gradients['W1'] = np.dot(np.transpose(X), grad)
        self.gradients['b1'] = np.sum(grad, axis = 0)
